...
export default function UnusualTradesClient() {
  ...
  useEffect(() => {
    if (!initialGu) return;
    setLoadingList(true);
    fetch(`/api/unusual-trades?gu=${encodeURIComponent(initialGu)}`)
      .then(r => r.json()).then(setUnusualTrades)
  ...
  const handleGuSelect = async (gu: string) => {
    ...
    setLoadingList(true);
    router.replace(`/unusual-trades?gu=${encodeURIComponent(gu)}`, { scroll: false });
    try {
  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#999', margin: 0 }}>구를 선택하면 최근 거래에서 큰 폭의 하락이 감지된 아파트 목록을 보여줍니다. 아파트명을 클릭하면 차트가 표시됩니다. 모든 면적은 전용면적입니다.</p>
      </div>
