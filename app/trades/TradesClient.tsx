...
export default function TradesClient() {
  ...
  const [gu, setGu] = useState(() => searchParams.get('gu') ?? '');
  ...
  const setUrl = useCallback((p: { gu?: string; dong?: string; apt?: string; area?: string }) => {
    const sp = new URLSearchParams();
    if (p.gu) sp.set('gu', p.gu);
    if (p.dong) sp.set('dong', p.dong);
    if (p.apt) sp.set('apt', p.apt);
    if (p.area) sp.set('area', p.area);
    const q = sp.toString();
    router.replace(q ? `/trades?${q}` : '/trades', { scroll: false });
  }, [router]);
  ...
  const handleGuChange = (v: string) => { setGu(v); setDong(''); setApt(''); setArea(''); setTrades([]); resetMode(); setUrl({ gu: v }); };
  const handleDongChange = (v: string) => { setDong(v); setApt(''); setArea(''); setTrades([]); resetMode(); setUrl({ gu, dong: v }); };
  const handleAptChange = (v: string) => { setApt(v); setArea(''); setTrades([]); resetMode(); setUrl({ gu, dong, apt: v }); };
  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#999', margin: 0 }}>아파트를 선택하고 함께보기를 눌러보세요! 옆단지 함께보기는 반경 750m 이내, 우리동네 함께보기는 같은 동 아파트의 비슷한 평수가 검색됩니다. 모든 면적은 전용면적입니다.</p>
      </div>
      ...
      <div className="filter-grid">
        ...
        <Select value={area} onChange={v => { setArea(v); resetMode(); setUrl({ gu, dong, apt, area: v }); }} options={areas.map(String)} placeholder="면적(㎡)" disabled={!apt || areas.length === 0} />
      </div>
