...
export default function NewHighsClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/new-highs?gu=${encodeURIComponent(initialGu)}`)
      .then(r => r.json()).then(setNewHighs)
      .finally(() => setLoadingNewHighs(false));
  ...
  const handleGuSelect = async (gu: string) => {
    ...
    setLoadingNewHighs(true);
    router.replace(`/new-highs?gu=${encodeURIComponent(gu)}`, { scroll: false });
    try {
  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#999', margin: 0 }}>구를 선택하면 최근 거래가 신고가인 아파트 목록을 보여줍니다. 아파트명을 클릭하면 차트가 표시됩니다. 모든 면적은 전용면적입니다.</p>
      </div>
