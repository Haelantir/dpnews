...
export default function TopAptsClient() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialGu = searchParams.get('gu') ?? '';
  const initAt = searchParams.get('areaType');
  const initialAreaType: AreaType = (initAt === '59' || initAt === '84' || initAt === '100+') ? initAt : '84';

  const [filterData, setFilterData] = useState<FilterData | null>(null);
  const [selectedGu, setSelectedGu] = useState(initialGu);
  const [areaType, setAreaType] = useState<AreaType>(initialAreaType);
  ...
  const buildUrl = (g: string, at: AreaType) => {
    const sp = new URLSearchParams();
    if (g) sp.set('gu', g);
    sp.set('areaType', at);
    return g ? `/top-apts?${sp.toString()}` : '/top-apts';
  };

  // Auto-load on mount if gu is in URL
  useEffect(() => {
    if (!initialGu) return;
    fetchData(initialGu, initialAreaType);
  ...
  const handleGuSelect = (gu: string) => {
    ...
    router.replace(buildUrl(gu, areaType), { scroll: false });
  };
  ...
  const handleAreaType = (at: AreaType) => {
    ...
    router.replace(buildUrl(selectedGu, at), { scroll: false });
  };
  ...
  return (
    <div className="page-wrap">
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#999', margin: 0 }}>이 지역의 평당 단가로 계산된 대장 순위입니다. 59, 84, 대형을 선택할 수 있습니다. 모든 면적은 전용면적입니다.</p>
      </div>
            ...
            <div className="wide-layout">
              <div className="wide-col">
                ...
                <div style={{ overflowY: 'auto', maxHeight: 640 }}>
                  <table className="top-apts-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead style={{ position: 'sticky', top: 0, background: '#fff', zIndex: 1 }}>
                      <tr>
                        {([['순위', 'col-rank'], ['아파트명', ''], ['동', 'col-dong'], ['만원/㎡', 'col-price']] as [string, string][]).map(([h, cls]) => (
                          <th key={h} className={cls} style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 700, whiteSpace: 'nowrap', borderBottom: '2px solid #111' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {aptData!.table.map((apt, i) => (
                        <tr key={`${apt.aptNm}|${apt.dong}`}
                          ...
                          <td className="col-rank" style={{ padding: '6px 8px', color: '#888', fontSize: 12 }}>{i + 1}</td>
                          <td style={{ padding: '6px 8px', fontWeight: i < 3 ? 700 : 400 }}>{apt.aptNm}</td>
                          <td className="col-dong" style={{ padding: '6px 8px', color: '#555' }}>{apt.dong}</td>
                          <td className="col-price" style={{ padding: '6px 8px', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>
